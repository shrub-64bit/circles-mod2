...
function progress(i, x){
    ...
    progressValues[i]=x;
    valueContainer.textContent = `${format(progressValues[0])*4}%`
...
function makeCircleEffectText(){
    ...
    for (let i = 2; i < data.numbers.length; i++) {
        string += `Circle ${i+1} (${formatWhole(data.numbers[i])*4}%): ${format(higherEffects[i-2])}x to the power of Circle ${i}.\n`
    }
    return string
}
// space filling
function circleTextControls(){
    ...
    document.getElementById("descText").innerText = `Circle 2 (${formatWhole(data.numbers[1])*4}%) provides a ${format(effect)}x boost to the speed of Circle 1.`
...
// space filling
    ...
    document.getElementById("descText4").innerText = `Circle 10 (${formatWhole(data.numbers[9])*4}%) increases the minimum percent of all lower Circles by ${format(minimumNumber(0).sub(1))*4}%`
}
