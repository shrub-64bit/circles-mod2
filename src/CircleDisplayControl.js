...
function createBars(su=false){
    if(data.numbers[data.numbers.length-1].gte(100) || su){
        ...
        newBar.style.height = `${180+(progressBars.length)*10}px`// this bit is cursed
        newBar.style.width = `${180+(progressBars.length)*10}px` // so is this
...
function makeCircleEffectText(){
    ...
    for (let i = 2; i < data.numbers.length; i++) {
        string += `Circle ${i+1} (${formatWhole(data.numbers[i])}%): ${format(higherEffects[i-2])}x to the power of Circle ${i}.\n` // adding how circle n+1 affects circle n
    }
    return string
}
// space filling
function circleTextControls(){
    ...
    document.getElementById("descText").innerText = `Circle 2 (${formatWhole(data.numbers[1])}%) provides a ${format(effect)}x boost to the speed of Circle 1.` // circle 2's effect on circle 1
...
// space filling
    ...
    document.getElementById("descText4").innerText = `Circle 10 (${formatWhole(data.numbers[9])}%) increases the minimum percent of all lower Circles by ${format(minimumNumber(0).sub(1))}%` // the last circle
}
